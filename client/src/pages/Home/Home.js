...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    Movies.getMy(sessionStorage.getItem("userId"))
      .then(myFavoriteMovies => {
        ...
        setMovieResult(myFavoriteMovies.data[myFavoriteMovies.data.length - 1]);
        setHDPoster(myFavoriteMovies.data[myFavoriteMovies.data.length - 1].Poster.replace("X300", ""));
      })
  ...
  return (
    <>
      ...
      {user ? 
      (<Dashboard>
          <NavBar setLoggedUser={getUser} user={sessionStorage.getItem("email")} setMovieResult={setMovieResult} setHDPoster={setHDPoster}/>
          {
          movieResult ? 
          (<>
              <DesktopWidth>
                <SearchResultHeader poster={HDPoster} movieResult={movieResult} handleFavorite={handleFavorite} />
                <SearchResultSummary featuredMovie={movieResult} />
                {favorites.length && window.innerWidth > 1025 ? (<Favorites favs={favorites} setMovieResult={setMovieResult} setHDPoster={setHDPoster}/>) : (<></>)}
              </DesktopWidth>
              {favorites.length && window.innerWidth < 1025 ? (<Favorites favs={favorites} setMovieResult={setMovieResult} setHDPoster={setHDPoster}/>) : (<></>)}
          </>) : 
          (<SearchHeader setMovieResult={setMovieResult}setHDPoster={setHDPoster}/>)
          }
      </Dashboard>) : 
